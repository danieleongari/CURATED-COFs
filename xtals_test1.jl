...
# ╔═╡ 6af6b83e-1292-11ed-044d-2d50956e2c3b
using Xtals

# ╔═╡ b2ec685f-39f6-4cfa-b572-b2ddda864ff2
rc[:paths][:crystals] = joinpath(pwd(), "cifs")

# ╔═╡ 408b2d9e-d49c-4302-b118-c222933ec359
xtal_files = readdir(rc[:paths][:crystals])

# ╔═╡ 62db186e-3642-4d3e-a52b-7c303d6b98d0
length(xtal_files)

# ╔═╡ 6e631e83-b602-40e7-847e-c932ed1bd1a9
begin
	bond_anomalies = String[]
	load_anomalies = String[]
	for xtal_file in xtal_files
		try
			xtal = Crystal(xtal_file)
			if !infer_bonds!(xtal, true)
			push!(bond_anomalies, xtal_file)
		end
		catch
			push!(load_anomalies, xtal_file)
		end
	end
end

# ╔═╡ 1c8b89c2-2abb-4a1b-8cfc-072bbd8cc086
length(bond_anomalies)

# ╔═╡ 73cc64ad-fbca-48dd-a0b1-6ba6bcd8234e
open("bond_anomalies.txt", "w") do f
	for xtal_name in bond_anomalies
		write(f, "$xtal_name\n")
	end
end

# ╔═╡ baddd4c7-063c-4062-b601-c26a8c6991fe
length(load_anomalies)

# ╔═╡ d2ece8f7-75e3-48a8-a7a1-4b01c536b7f4
load_anomalies

# ╔═╡ c27c6b7a-f5e5-452d-8d31-9450011f7fdb
Crystal("21192N2.cif")
